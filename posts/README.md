# Blog Posts

_Auto-generated during build. Do not edit manually._

## [My Agentic AI Development Workflow](my-agentic-AI-development-workflow.md)
> Nobody ever asks me how I use agentic AI development in my day to day life so I thought I would just write it all down and put it out into the world anyways.

## [The Four Agreements](the-four-agreements.md)
> So I was watching the show Barry and in season 2 a certain character mentioned the book 'The Four Agreements' and I had to pause and look it up to see if it was real. Sadly the book 'Hit Your Mark and Say Your Lines' by Gene Cousineau isn't real but it turned out 'The Four Agreements' by Don Miguel Ruiz is very much real so I went ahead and hit buy now.

## [AI Clothing Assistant](ai-clothing-assistant.md)
> So I have had an idea bouncing around in my head for a few months now. I moved recently and also rearranged my closet and both times I thought, man I really should catalog all this while I'm touching each one.

## [Atomic Habits - Book Review](atomic-habits.md)
> I was at the Tattered Cover store in Union Station looking at books with the kids when I saw a copy of Atomic Habits on the shelf.

## [Escaping the AI Feedback Loop: Tips for Devs](escaping-the-ai-feedback-loop.md)
> Discover strategies to break free from repetitive AI coding loops and learn when to leverage traditional resources for efficient problem-solving.

## [Hello World - Welcome to TheDigital.Ninja](first-post.md)
> Kicking off my new tech blog with a classic Hello World. Join me as I explore app dev, architecture, AI and more.
